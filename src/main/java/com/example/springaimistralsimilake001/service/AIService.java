...
public class AIService {
    ...
    public List<Document> search(String query) {
        // List<Document> results = vectorStore.doSimilaritySearch(SearchRequest.query(query).withTopK(3));
        FilterExpressionBuilder filter = new FilterExpressionBuilder();
        List<Document> results = vectorStore.doSimilaritySearch(SearchRequest.query(query)
                .withTopK(5)
